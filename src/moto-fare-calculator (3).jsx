...
function PlacesSearchBox({ label, placeholder, color, inputRef, onPlaceSelect }) {
  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{
        fontSize: 11, fontWeight: 600, color: accent, marginBottom: 6,
      ...
      <div style={{
        ...
        borderRadius: 10, background: COLORS.card, overflow: "hidden",
        transition: "border-color 0.15s",
      }}>
        <div style={{
          padding: "0 12px", color: accent,
        ...
        />
        {hasValue && <div style={{ padding: "0 12px", color: COLORS.green, fontSize: 16 }}>✓</div>}
      </div>
    </div>
  );
}
...
export default function MotoCalc() {
  ...
  const distanceService = useRef(null);
  ...
  useEffect(() => {
    ...
    const kigaliBounds = new window.google.maps.LatLngBounds(
      new window.google.maps.LatLng(-2.05, 29.95),
      new window.google.maps.LatLng(-1.85, 30.20)
    );

    fromAutocomplete.current = new window.google.maps.places.Autocomplete(
      fromInputRef.current,
      { bounds: kigaliBounds, strictBounds: false, componentRestrictions: { country: "rw" } }
    );
    toAutocomplete.current = new window.google.maps.places.Autocomplete(
      toInputRef.current,
      { bounds: kigaliBounds, strictBounds: false, componentRestrictions: { country: "rw" } }
    );
    ...
    fromAutocomplete.current.addListener("place_changed", () => {
      ...
      if (place.geometry) setFromPlace(place);
    });

    toAutocomplete.current.addListener("place_changed", () => {
      const place = toAutocomplete.current.getPlace();
      if (place.geometry) setToPlace(place);
    });
  }, [mapsReady]);

  useEffect(() => {
    if (!fromPlace || !toPlace || !distanceService.current) return;
      ...
      (response, status) => {
        if (status === "OK") {
          ...
          if (element.status === "OK") {
            ...
            const realKm = Math.max(1, Math.min(20, Math.round(meters / 1000)));
            setKm(realKm);
            setDistanceLabel(element.distance.text + " by road");
          }
        }
      }
    );
    ...
    <div style={{
      ...
      <div style={{ padding: "28px 20px 20px", borderBottom: `1px solid ${COLORS.border}` }}>
        ...
        <p style={{ fontSize: 13, color: COLORS.muted, margin: 0, lineHeight: 1.5 }}>
          Search any landmark or street in Kigali
        </p>
      </div>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        <div style={{ marginTop: 20, marginBottom: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          <PlacesSearchBox
            label="From"
          ...
          {distanceLabel && (
            <div style={{
              ...
              <span style={{ fontSize: 13, color: COLORS.green, fontFamily: "'DM Mono', monospace", fontWeight: 600 }}>
                {km} km
              </span>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, letterSpacing: "0.04em" }}>
              {fromPlace && toPlace ? "Distance (adjust if needed)" : "Or set distance manually"}
            </span>
            <span style={{
              fontSize: 15, fontWeight: 600, color: COLORS.green,
              fontFamily: "'DM Mono', monospace",
              background: COLORS.greenDim, padding: "2px 10px",
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: COLORS.greenDim, filter: "blur(30px)",
          }} />
        ...
        {/* Kinyarwanda phrase */}
        <div style={{
          background: "#111A13", border: `1px solid #1E3A22`,
      ...
      <style>{`
        ...
        .pac-container { background: ${COLORS.card} !important; border: 1px solid ${COLORS.border} !important; border-radius: 10px !important; margin-top: 4px !important; font-family: 'DM Sans', sans-serif !important; }
        .pac-item { padding: 8px 14px !important; color: ${COLORS.text} !important; border-top: 1px solid ${COLORS.border} !important; cursor: pointer !important; font-size: 13px !important; }
        .pac-item:hover { background: ${COLORS.surface} !important; }
        .pac-item-query { color: ${COLORS.text} !important; font-size: 13px !important; }
        .pac-matched { color: ${COLORS.green} !important; }
